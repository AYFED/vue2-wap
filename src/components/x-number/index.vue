<template>
  <div class="ayui-cell">
    ...
    <div class="ayui-cell__ft ayui-cell-primary" :class="{'ayui-number-round': buttonStyle === 'round'}" v-show="!readonly" style="font-size:0">
      <div :style="{float:align}">
        <a @click="sub" class="ayui-number-selector ayui-number-selector-sub":class="{'ayui-number-disabled':disabledMin}">
        ...
        <input v-model.number="currentValue" :name="name" class="ayui-number-input" :style="{width: width}" :readonly="!fillable" pattern="[0-9]*" type="number" @blur="blur" />
        <a @click="add" class="ayui-number-selector ayui-number-selector-plus" :class="{'ayui-number-disabled':disabledMax}">
    ...
    <div class="ayui-cell__ft ayui-cell-primary" v-show="readonly">
      {{value}}
    </div>
  </div>
</template>
...
.ayui-number-round .ayui-number-input {
  border: none;
}
.ayui-number-selector {
  float:left;
  height:20px;
  font-size:25px;
  line-height:18px;
...
.ayui-number-round .ayui-number-selector {
  width: 20px;
  border-radius: 13px;
}
.ayui-number-selector svg {
  fill: @number-button-font-color;
}
.ayui-number-selector.ayui-number-disabled svg {
  fill: #ccc;
}
.ayui-number-round .ayui-number-selector.ayui-number-disabled {
  border-color: @number-round-button-disabled-border-color;
}
.ayui-number-round .ayui-number-selector.ayui-number-disabled svg {
  fill: #ccc;
}
.ayui-number-selector-sub {
  border-right:none;
  padding:4px 8px 2px 8px;
  border-radius:2px 0 0 2px;
}
.ayui-number-selector-plus {
  border-left:none;
  margin-right: 5px;
  padding:3px 8px;
  border-radius:0 2px 2px 0;
}
.ayui-number-round .ayui-number-selector-sub svg {
  position: relative;
  top: 1px;
}
.ayui-number-round .ayui-number-selector-sub, .ayui-number-round .ayui-number-selector-plus {
  padding: 2px;
