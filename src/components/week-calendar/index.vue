<template>
    <div class="ayui-week-calendar">
        <slot name="header" :current-month="currentMonth"></slot>
        <swiper
                class="ayui-week-calendar"
                v-model="index"
                :show-dots="false"
                :height="height"
            ...
            <template v-for="(item, index) in [lastWeekList, currentWeekList, nextWeekList]">
                <swiper-item :data-start="item[0].format" :id="`x000${index}`">
                    <slot :name="`header-day-list-${index}`">
                        <flexbox :gutter="0">
                            <flexbox-item
                                    class="ayui-week-calendar-day-item"
                    ...
                    <flexbox :gutter="0">
                        <flexbox-item
                                class="ayui-week-calendar-date-item"
                                @click.native="onClick(i)"
                                :class="{
                                    'is-weekend': i.isWeekend,
                                    'ayui-week-calendar-is-today': i.isToday,
                                    'ayui-week-calendar-is-selected': i.format === value
                                  }"
                                v-for="(i, itemIndex) in item"
                                :key="itemIndex"
                                :data-date="i.format">
                            <div class="ayui-week-calendar-each-day-box">
                                <slot
                    ...
                    <span class="ayui-week-calendar-each-day" :style="getMarkStyle(i)">
                    {{ i.str }}
                      <span class="ayui-week-calendar-top-tip" v-if="isShowTopTip(i)" :style="isShowTopTip(i, 'style')">
                        <span>{{ isShowTopTip(i, 'text') }}</span>
                      </span>
                    </span>
                                    <span class="ayui-calendar-dot" v-show="isShowBottomDot(i)"></span>
                                </slot>
                            </div>
                        </flexbox-item>
                    </flexbox>
...
<style lang="less" scoped>
    ...
    .ayui-week-calendar-day-item, .ayui-week-calendar-date-item {
        text-align: center;
    }

    .ayui-week-calendar-day-item {
        color: @week-calendar-header-day-item-color;
    }

    .ayui-week-calendar-each-day-box {
        padding: 5px 0;
    }
    ...
    .ayui-week-calendar-is-selected .ayui-week-calendar-each-day {
