<template>
    ...
    <div class="ayui-cell__hd">
      ...
      <slot name="label">
        <label class="ayui-label" :class="labelClass" :style="{width: labelWidth || $parent.labelWidth || labelWidthComputed, textAlign: $parent.labelAlign, marginRight: $parent.labelMarginRight}" v-if="title" v-html="title" :for="`ayui-x-input-${uuid}`"></label>
        <inline-desc v-if="inlineDesc">{{inlineDesc}}</inline-desc>
      </slot>
    </div>
    <div class="ayui-cell__bd ayui-cell__primary" :class="placeholderAlign ? `ayui-x-input-placeholder-${placeholderAlign}` : ''">
      <input
      :id="`ayui-x-input-${uuid}`"
      v-if="!type || type === 'text'"
      class="ayui-input"
      :maxlength="max"
    ...
    <div class="ayui-cell__ft">
      ...
      <icon type="success" class="ayui-input-icon" v-show="novalidate && iconType === 'success'"></icon>
...
.ayui-x-input .ayui-x-input-placeholder-right input::-webkit-input-placeholder {
  text-align: right;
}
.ayui-x-input .ayui-x-input-placeholder-center input::-webkit-input-placeholder {
  text-align: center;
}
.ayui-x-input .ayui-input-icon {
  font-size: 21px;
}
.ayui-input-icon.ayui-icon-warn:before, .ayui-input-icon.ayui-icon-success:before {
  font-size: 21px;
}
.ayui-x-input .ayui-icon {
  padding-left: 5px;
}
