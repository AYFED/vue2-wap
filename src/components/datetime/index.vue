<template>
  <a
  class="ayui-datetime ayui-cell"
  :class="{'ayui-cell_access': !readonly}"
  ...
  href="javascript:">
    <slot>
      ...
      <div class="ayui-cell__ft ayui-cell-primary ayui-datetime-value" :style="{textAlign: valueTextAlign}">
        <span class="ayui-cell-placeholder" v-if="!currentValue && placeholder">{{ placeholder }}</span>
        <span class="ayui-cell-value" v-if="currentValue">{{ displayFormat ? displayFormat(currentValue) : currentValue }}</span>
        <icon class="ayui-input-icon" type="warn" v-show="!valid" :title="firstError"></icon>
      </div>
    </slot>
  </a>
</template>
...
export default {
  ...
  mounted () {
    ...
    this.$el.setAttribute('id', `ayui-datetime-${uuid}`)
    if (!this.readonly) {
      this.$nextTick(() => {
        this.render()
  ...
  computed: {
    ...
    pickerOptions () {
      const _this = this
      const options = {
        trigger: '#ayui-datetime-' + this.uuid,
        format: this.format,
        value: this.currentValue,
        output: '.ayui-datetime-value',
