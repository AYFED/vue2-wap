...
export default function (nav, options = {}) {
  ...
  const getFillElem = function (el) {
    ...
    if (next.classList.contains('ayui-sticky-fill')) {
      return next
    }
  ...
  const scrollHandler = function () {
    ...
    if (distance > navOffsetY) {
      ...
      nav.classList.add('ayui-fixed')
    } else {
      nav.classList.remove('ayui-fixed')
    }
  }
  ...
  if (checkStickySupport && (gtIOS6() || isSupportSticky())) {
    nav.style.top = offset + 'px'
    // 大于等于iOS6版本使用sticky
    nav.classList.add('ayui-sticky')
  } else {
    if (nav.classList.contains('ayui-fixed')) {
      const top = getTop()
