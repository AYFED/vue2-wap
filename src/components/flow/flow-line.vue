<template>
  <div class="ayui-wepay-flow__line" :class="classes" :style="styles">
    <div class="ayui-wepay-flow__process" :style="{width: getWidth(this.processSpan)}"></div>
    <div v-if="tip" :class="`ayui-wepay-flow__info-${finalTipDirection}`" :style="{left: getWidth(this.processSpan)}">{{ tip }}</div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    classes () {
      if (this.isDone) {
        return 'ayui-wepay-flow__line_done'
      }
      if (!this.isDone && this.tip) {
        return 'ayui-wepay-flow__line_ing'
      }
    }
  }
}
