<template>
  <div :class="disabled ? 'ayui-checklist-disabled' : ''">
    <div v-show="title" class="ayui-cells__title">{{ title }}</div>
    <slot name="after-title"></slot>
    <div class="ayui-cells ayui-cells_checkbox">
      <label class="ayui-cell ayui-check_label" :class="{'ayui-checklist-label-left': labelPosition === 'left'}" :for="`checkbox_${uuid}_${index}`" v-for="(one, index) in currentOptions">
        <div class="ayui-cell__hd">
          <input type="checkbox" class="ayui-check" :name="`ayui-checkbox-${uuid}`" :value="getKey(one)" v-model="currentValue" :id="disabled ? '' : `checkbox_${uuid}_${index}`" :disabled="isDisabled(getKey(one))">
          <i class="ayui-iconfont ayui-icon-checked ayui-checklist-icon-checked"></i>
        </div>
...
.ayui-cells_checkbox .ayui-check:checked + .ayui-checklist-icon-checked:before {
  color: @checklist-icon-active-color;
}

.ayui-cells_checkbox > label > * {
  pointer-events: none;
}
.ayui-checklist-disabled .ayui-checklist-icon-checked:before {
  opacity: 0.5;
}
.ayui-checklist-label-left {
