    ...
    <div class="calendar-header" v-show="!hideHeader">
      <div class="calendar-year">
        <span @click="go(year - 1, month)">
          <a class="year-prev ayui-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span class="calendar-header-right-arrow" @click="go(year + 1, month)">
          <a class="year-next ayui-next-icon" href="javascript:"></a>
        </span>
      </div>

      <div class="calendar-month">
        <span @click="prev">
          <a class="month-prev ayui-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span @click="next" class="calendar-header-right-arrow">
          <a class="month-next ayui-next-icon" href="javascript:"></a>
        </span>
      </div>
    </div>
      ...
      <tbody>
        <tr v-for="(day,k1) in days">
          ...
          @click="select(k1, k2, child)">
            ...
            class-name="ayui-calendar-each-date"
            :row="k1"
            :col="k2"
            ...
            name="each-day">
              <span
              class="ayui-calendar-each-date"
              :style="getMarkStyle(child)"
              v-show="showChild(year, month, child)">
                {{ replaceText(child.day, formatDate(year, month, child)) }}
                <span class="ayui-calendar-top-tip" v-if="isShowTopTip(child)" :style="isShowTopTip(child, 'style')">
                  <span>{{ isShowTopTip(child, 'text') }}</span>
                </span>
              </span>
              <span class="ayui-calendar-dot" v-if="isShowBottomDot(child)"></span>
...
.inline-calendar td > span.ayui-calendar-each-date {
  position: relative;
  display: inline-block;
...
.inline-calendar td.current > span.ayui-calendar-each-date {
