<template>
  <img :src="defaultSrc" :data-src="currentSrc" class="ayui-x-img"/>
</template>
...
export default {
  ...
  mixins: [uuidMixin],
  created () {
    this.$ayui && this.$ayui.bus && this.$ayui.bus.$on('ayui:after-view-enter', this.init)
  },
  methods: {
    init () {
      const _this = this
  ...
  mounted () {
    this.$el.setAttribute('id', `ayui-ximg-${this.uuid}`)
    this.$nextTick(() => {
      setTimeout(() => {
        this.init()
      }, this.delay)
  ...
  beforeDestroy () {
    this.blazy && this.blazy.destroy()
    this.blazy = null
    this.$ayui && this.$ayui.bus && this.$ayui.bus.$off('ayui:after-view-enter', this.init)
  }
}
</script>
