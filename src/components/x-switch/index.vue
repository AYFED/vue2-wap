<template>
  <div class="ayui-x-switch ayui-cell ayui-cell_switch">
    <div class="ayui-cell__bd">
      <label class="ayui-label" :style="labelStyle" :class="labelClass" v-html="title"></label>
      <inline-desc v-if="inlineDesc">{{ inlineDesc }}</inline-desc>
    </div>
    <div class="ayui-cell__ft">
      <input class="ayui-switch" type="checkbox" :disabled="disabled" v-model="currentValue" />
      <div v-if="preventDefault" class="ayui-x-switch-overlay" @click="onClick"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    labelClass () {
      return {
        'ayui-cell-justify': this.$parent && this.$parent.labelAlign === 'justify'
      }
    }
  },
  methods: {
...
input.ayui-switch[disabled] {
  opacity: @switch-disabled-opacity;
}

.ayui-x-switch.ayui-cell_switch {
  padding-top: 6px;
  padding-bottom: 6px;
}

.ayui-x-switch-overlay {
  width: 60px;
  height: 50px;
  position: absolute;
  right: 0;
