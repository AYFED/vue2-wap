<template>
  <div class="ayui-slider">
    <div class="ayui-swiper" :style="{height: xheight}">
      <slot></slot>

      <div
      class="ayui-swiper-item"
      v-for="(item, index) in list"
      @click="clickListItem(item)"
      :data-index="index">
        <a href="javascript:">
          <div class="ayui-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="ayui-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="ayui-swiper-item ayui-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'ayui-indicator', `ayui-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i class="ayui-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
