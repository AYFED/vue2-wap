...
if (isBrowser) {
  window.__$ayuiPopups = window.__$ayuiPopups || {}
}
...
const popupDialog = function (option) {
  ...
  if (!!document.querySelectorAll('.ayui-popup-mask').length <= 0) {
    this.divMask = document.createElement('a')
    this.divMask.className = 'ayui-popup-mask'
  ...
  div.className += ` ayui-popup-dialog ayui-popup-dialog-${this.uuid}`
  if (!this.params.hideOnBlur) {
    div.className += ' ayui-popup-mask-disabled'
  }
  ...
  this.container = document.querySelector('.ayui-popup-dialog-' + this.uuid)
  this.mask = document.querySelector('.ayui-popup-mask')
...
popupDialog.prototype.show = function () {
  if (this.params.showMask) {
    this.mask.classList.add('ayui-popup-show')
    this.mask.style['zIndex'] = 500
  }
  this.container.classList.add('ayui-popup-show')
  this.params.onOpen && this.params.onOpen(this)
  if (isBrowser) {
    window.__$ayuiPopups[this.uuid] = 1
  }
}

popupDialog.prototype.hide = function (shouldCallback = true) {
  this.container.classList.remove('ayui-popup-show')
  if (!document.querySelector('.ayui-popup-dialog.ayui-popup-show')) {
    this.mask.classList.remove('ayui-popup-show')
    setTimeout(() => {
  ...
  this.isShow = false
  if (isBrowser) {
    delete window.__$ayuiPopups[this.uuid]
  }
}
