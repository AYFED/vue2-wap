<template>
  <div>
    ...
    <group :title="$t('Use is-link to show arrow')">
      <cell is-link>
        <span slot="title" style="color:#F83616;"><span style="vertical-align:middle;">{{ $t('Messages') }}</span> <badge text="1"></badge></span>
      </cell>
    ...
    <group label-width="5em" title="对齐模式">
    ...
    <group :title="$t('Use slot for complicated content')">
      <cell :title="$t('Slot content')">
        <div>
          <span style="color: #F83616;font-size: 14px;">{{$t('Hi I am AYUI')}}</span>
        </div>
      </cell>
    </group>
    ...
    <group :title="$t('is-link is set to true automatically when link exists')">
      ...
      <cell :title="$t('Http link')" link="https://m.aoyou.com" inline-desc='link="https://m.aoyou.com"'></cell>
    </group>
...
<script>
import { Cell, CellBox, CellFormPreview, Group, Badge } from '@/components'
