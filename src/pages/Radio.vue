<template>
  <div>
    ...
    <group :title="$t('fill mode value is ')+radio001Value">
    ...
    <group title="slot:each-item">
      <radio :options="radio001" @on-change="change">
        <template slot-scope="props" slot="each-item"><!-- use scope="props" when vue < 2.5.0 -->
          <p>
            V{{ props.index + 1 }} <img src="http://dn-placeholder.qbox.me/110x110/FF2D55/000" class="ayui-radio-icon"> {{ props.label }}
          </p>
        </template>
      </radio>
    </group>
